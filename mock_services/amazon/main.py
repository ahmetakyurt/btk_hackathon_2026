"""Mock Amazon SP-API (port 9002).

Simulates Amazon's marketplace with:
- ASIN-style external_id (B0 + 8 alphanumeric).
- Fulfillment field (FBA/FBM) on every listing.
- 4 seeded competitors per listing in [-6%, +6%] band.
- Buybox awarded to lowest-priced FBA listing first; FBM only wins if no FBA exists.
"""
...
DB_URL = os.getenv("MOCK_AMAZON_DB", "sqlite+aiosqlite:///./mock_amazon.db")
...
def _make_external_id() -> str:
    """ASIN-style: B0 + 8 random alphanumerics."""
    rest = "".join(random.choices(string.ascii_uppercase + string.digits, k=8))
    return f"{EXTERNAL_ID_PREFIX}{rest}"
...
def _seed_competitors(listing_price: Decimal) -> list[Competitor]:
    ...
    for name in chosen:
        delta_pct = Decimal(str(random.uniform(-0.06, 0.06)))
...
app = FastAPI(
    title="Mock Amazon SP-API",
    version="0.1.0",
    description="Simulated Amazon SP-API for OptiPrice AI.",
    lifespan=lifespan,
)
...
async def list_product(
    ...
    if body.fulfillment not in ("FBA", "FBM"):
        raise HTTPException(status_code=400, detail="fulfillment must be FBA or FBM")

    # Retry on rare ASIN collision (8 random chars → 36^8 space, near-impossible).
    for _ in range(5):
        external_id = _make_external_id()
        exists = (
            await session.execute(
                select(Listing.id).where(Listing.external_id == external_id)
            )
        ).scalar_one_or_none()
        if exists is None:
            break
    else:
        raise HTTPException(status_code=500, detail="Could not allocate ASIN")
...
async def _get_listing_or_404(
    ...
    if listing is None:
        raise HTTPException(status_code=404, detail=f"ASIN {external_id} not found")
    return listing
